...
###################################################################################################                                                                                                 
#LLLLLLLLLLL                SSSSSSSSSSSSSSS TTTTTTTTTTTTTTTTTTTTTTTMMMMMMMM               MMMMMMMM#
#L:::::::::L              SS:::::::::::::::ST:::::::::::::::::::::TM:::::::M             M:::::::M#
#L:::::::::L             S:::::SSSSSS::::::ST:::::::::::::::::::::TM::::::::M           M::::::::M#
#LL:::::::LL             S:::::S     SSSSSSST:::::TT:::::::TT:::::TM:::::::::M         M:::::::::M#
#  L:::::L               S:::::S            TTTTTT  T:::::T  TTTTTTM::::::::::M       M::::::::::M#
#  L:::::L               S:::::S                    T:::::T        M:::::::::::M     M:::::::::::M#
#  L:::::L                S::::SSSS                 T:::::T        M:::::::M::::M   M::::M:::::::M#
#  L:::::L                 SS::::::SSSSS            T:::::T        M::::::M M::::M M::::M M::::::M#
#  L:::::L                   SSS::::::::SS          T:::::T        M::::::M  M::::M::::M  M::::::M#
#  L:::::L                      SSSSSS::::S         T:::::T        M::::::M   M:::::::M   M::::::M#
#  L:::::L                           S:::::S        T:::::T        M::::::M    M:::::M    M::::::M#
#  L:::::L         LLLLLL            S:::::S        T:::::T        M::::::M     MMMMM     M::::::M#
#LL:::::::LLLLLLLLL:::::LSSSSSSS     S:::::S      TT:::::::TT      M::::::M               M::::::M#
#L::::::::::::::::::::::LS::::::SSSSSS:::::S      T:::::::::T      M::::::M               M::::::M#
#L::::::::::::::::::::::LS:::::::::::::::SS       T:::::::::T      M::::::M               M::::::M#
#LLLLLLLLLLLLLLLLLLLLLLLL SSSSSSSSSSSSSSS         TTTTTTTTTTT      MMMMMMMM               MMMMMMMM#
###################################################################################################                                                                                                 



#THIS METHOD CAN BE USED TO DIFFERENTIATE BETWEEN P = 2*P_TRUE AND P = P_TRUE 
#DATA SET A = ALL CORRECTELY FOLDED PERIODICS
#DATA SET B = EXACT SAME DATA BUT FOLDED AT 2P OR 0.5P 
#PERFECT FOR RESOLVING HARMONICS!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
def hyper_params():
	...
	model_path = model_parent_dir + '/final_better/'
	big_plot_IO = -100
...
def LC_train(TOOL, method= 'PDM', N=200):
				...
				if random.uniform(0,100) < 10: 
					...
					if random.uniform(0,100) > 40: 
						...
						for i, m in enumerate(mag):
							mm = abs(m - med_mag)
							if phase[i] > 0.5:
								mm = mm + (scatter * minplus) + random.uniform(0,med_magerr) *0.5
							else:
								mm = mm - (scatter * minplus) + random.uniform(0,med_magerr) *0.5
	...
	if make_data == 1:
		...
		#=============#
		fp_real = 'ZTF'
		real_data_path = '/beegfs/car/njm/useless_OUTPUT/ZTF/ZTF'
		REAL_SAMPLE_FILE = np.loadtxt(real_data_path+'.csv', comments = '#', delimiter = ",").T
		real_names = REAL_SAMPLE_FILE[0]
		real_periods = REAL_SAMPLE_FILE[29]
		real_amplitudes = REAL_SAMPLE_FILE[8]
		real_samples = len(real_amplitudes)
		#real_samples = samps
		real_indexes = list(range(0,real_samples))
		random.shuffle(real_indexes)
		print("Loading this many samples of ZTF data:",real_samples, "\n\tAvailable:",len(real_amplitudes))
		for ii in tqdm(range(real_samples)):
			i = real_indexes[ii]
			TOOL.name = method + str(i)
			for mod in [0,1]:
				period = real_periods[i]
				name = real_names[i]
				amplitude = real_amplitudes[i]
				data_add(TOOL, method, fp_real, name, period, amplitude, knn, N = N, mod = mod, synth = 0)
		...
		ebsig_100_samples = 1000#len(sig_100_amplitudes)
		#ebsig_100_samples = samps
		sig_100_indexes = list(range(0,ebsig_100_samples))
		random.shuffle(sig_100_indexes)
		print("Loading this many samples of Eclipsing Binary sig_100 data:",ebsig_100_samples, "\n\tAvailable:",len(sig_100_amplitudes))
		for ii in tqdm(range(ebsig_100_samples)):
			i = sig_100_indexes[ii]
			TOOL.name = method + str(i)
			for mod in [0,1]:
				period = sig_100_periods[i]
				name = sig_100_names[i]
				amplitude = sig_100_amplitudes[i]
				data_add(TOOL, method, sig_100_fp, name, period, amplitude, knn, N = N, mod = mod, synth = synth, EB_mod = 1)
		...
		sig_100_amplitudes = sig_100_SAMPLE_FILE[1]		#THIS WILL BREAK WHEN OUTPUT IS CHANGED!!!!!!!!!!!!!!!!!!!!
		sig_100_periods = sig_100_SAMPLE_FILE[2]
		sig_100_samples = 7000#len(sig_100_amplitudes)
		#sig_100_samples = samps
		sig_100_indexes = list(range(0,sig_100_samples))
		random.shuffle(sig_100_indexes)
		print("Loading this many samples of sig_100 data:",sig_100_samples, "\n\tAvailable:",len(sig_100_amplitudes))
		for ii in tqdm(range(sig_100_samples)):
			i = sig_100_indexes[ii]
			TOOL.name = method + str(i)
			for mod in [0,1]:
				period = sig_100_periods[i]
				name = sig_100_names[i]
				amplitude = sig_100_amplitudes[i]
				data_add(TOOL, method, sig_100_fp, name, period, amplitude, knn, N = N, mod = mod, synth = synth)

		#=============#
		#ADD FAKE DATA#
		#=============#
		d001_fp = 'd001'
		d001_path = '/beegfs/car/njm/useless_OUTPUT/d001_LSTM/d001_LSTM'
		d001_SAMPLE_FILE = np.loadtxt(d001_path+'.csv', comments = '#', delimiter = ",").T
		d001_names = d001_SAMPLE_FILE[0]
		d001_periods = d001_SAMPLE_FILE[7]
		d001_amplitudes = d001_SAMPLE_FILE[8]		#THIS WILL BREAK WHEN OUTPUT IS CHANGED!!!!!!!!!!!!!!!!!!!!
		d001_samples = 6000#len(d001_amplitudes)
		#d001_samples = samps
		d001_indexes = list(range(0,d001_samples))
		random.shuffle(d001_indexes)
		print("Loading this many samples of D001 data:",d001_samples, "\n\tAvailable:",len(d001_amplitudes))
		for ii in tqdm(range(d001_samples)):
			i = d001_indexes[ii]
			TOOL.name = method + str(i)
			for mod in [0,1]:
				period = d001_periods[i]
				name = d001_names[i]
				amplitude = d001_amplitudes[i]
				data_add(TOOL, method, d001_fp, name, period, amplitude, knn, N = N, mod = mod, synth = synth)

		#=============#
		#ADD FAKE DATA#
		#=============#
		d002_fp = 'd002'
		d002_path = '/beegfs/car/njm/useless_OUTPUT/d002_LSTM/d002_LSTM'
		d002_SAMPLE_FILE = np.loadtxt(d002_path+'.csv', comments = '#', delimiter = ",").T
		d002_names = d002_SAMPLE_FILE[0]
		d002_periods = d002_SAMPLE_FILE[7]
		d002_amplitudes = d002_SAMPLE_FILE[8]		#THIS WILL BREAK WHEN OUTPUT IS CHANGED!!!!!!!!!!!!!!!!!!!!
		d002_samples = 1000#len(d002_amplitudes)
		#d002_samples = samps
		d002_indexes = list(range(0,d002_samples))
		random.shuffle(d002_indexes)
		print("Loading this many samples of D002 data:",d002_samples, "\n\tAvailable:",len(d002_amplitudes))
		for ii in tqdm(range(d002_samples)):
			i = d002_indexes[ii]
			TOOL.name = method + str(i)
			for mod in [0,1]:
				period = d002_periods[i]
				name = d002_names[i]
				amplitude = d002_amplitudes[i]
				data_add(TOOL, method, d002_fp, name, period, amplitude, knn, N = N, mod = mod, synth = synth)
